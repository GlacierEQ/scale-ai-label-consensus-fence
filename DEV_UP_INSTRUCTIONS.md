...
**Company lens (independent):** Scale AI (`scale_ai`)  
**Innovation:** Label Consensus Fence  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Scale AI affiliation, proprietary access, or production deployment.

### Bottleneck
Creating representative, contamination-resistant, non-gameable evaluations for agents and models.

### Brick wall
Measuring real capability and risk without overfitting, leaking benchmarks, or mistaking narrow scores for deployment readiness.

### Mechanism to implement
Promote labels only after consensus fence + rater provenance receipts; quarantine the rest.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Scale AI employment, endorsement, or proprietary systems access.
...
File: `src/label_consensus_fence.py`

- Expand `LabelConsensusFence` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_label_consensus_fence.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_label_consensus_fence.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Scale AI APIs
